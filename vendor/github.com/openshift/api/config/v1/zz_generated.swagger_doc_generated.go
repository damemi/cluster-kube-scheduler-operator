...
var map_SchedulerSpec = map[string]string{
	...
	"config":              "config is a reference to a ConfigMap containing a scheduler component config The Scheduler component config is beta as of k8s 1.19 and allows configuration of scheduler plugins and profiles, among other beta features. Note that this config will eventually replace the Policy config currently available Also note, that a component config can be passed with a Policy config as long as the component config does NOT contain any plugin configs, as these replace Policy configs and therefore can conflict.",
